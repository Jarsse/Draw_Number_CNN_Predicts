...
def create_model(model_name=None):
    ...
    if not model_name:
        act = "relu"
        model = Sequential([
            Conv2D(16, (3, 3), activation=act, input_shape=x_train.shape[1:]),
            MaxPooling2D(2, 2),
            Conv2D(32, (3, 3), activation=act, kernel_initializer='he_uniform'),
...
def train_and_save_model(dataX, dataY, n_folds=5):
    ...
    # fit model
    model.fit(trainX, trainY, epochs=150, batch_size=32, validation_data=(testX, testY), verbose=1, validation_split=0.25, callbacks=[es])
    model.save("savedModels/mnist_model_5")
    # evaluate model
